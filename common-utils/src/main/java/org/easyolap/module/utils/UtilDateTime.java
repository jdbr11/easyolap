...
public class UtilDateTime {
	...
	public static DateFormat toDateTimeFormat(String dateTimeFormat, TimeZone tz, Locale locale) {
		DateFormat df = null;
		if (null == dateTimeFormat || dateTimeFormat.length() == 0) {
	...
	public static DateFormat toTimeFormat(String timeFormat, TimeZone tz, Locale locale) {
		DateFormat df = null;
		if (null == timeFormat || timeFormat.length() == 0) {
	...
	public static int getNextHour(String strLongTime) {
		return getNextHour(Long.parseLong(strLongTime));
	}

	public static int getNextHour(long timeLong) {
		Calendar cal = getCalendarInstance(TimeZone.getTimeZone("GMT"), Locale.getDefault());
	...
	public static Date getDateNextHour(Date date) {
		Calendar calSeed = getCalendarInstance(TimeZone.getTimeZone("GMT"), Locale.getDefault());
		calSeed.setTime(date);
 
		calSeed.add(Calendar.HOUR, 1);
		calSeed.set(Calendar.MINUTE, 00);
		calSeed.set(Calendar.SECOND, 00);
		calSeed.set(Calendar.MILLISECOND, 000);
		return calSeed.getTime();
	}

	public static int getHour(String strLongTime) {
		long timeLong = Long.parseLong(strLongTime);
		Calendar cal = getCalendarInstance(TimeZone.getTimeZone("GMT"), Locale.getDefault());
		cal.setTimeInMillis(timeLong);
		String DATE_HOUR_FORMAT = "HH";
		SimpleDateFormat df = new SimpleDateFormat(DATE_HOUR_FORMAT);
		Date date = cal.getTime();
		return /* cal.get(Calendar.HOUR)*/ Integer.parseInt(df.format(date));
	}
	 ...
	 * @return
	 */
	public static boolean isSameHour(Date devontime, Date devofftime) {
		String DATE_HOUR_FORMAT = "yyyy-MM-dd HH";
		SimpleDateFormat df = new SimpleDateFormat(DATE_HOUR_FORMAT);
		boolean result = (df.format(devontime)).equals(df.format(devofftime));
		return result;
	}

	public static Date getDateByTimelong(String strLongTime) {
		long timeLong = Long.parseLong(strLongTime);
		Calendar cal = getCalendarInstance(TimeZone.getTimeZone("GMT"), Locale.getDefault());
		cal.setTimeInMillis(timeLong);
		return cal.getTime();
	...
	public static double getIntervalByMinute(Date from, Date thru) {
		double d = thru != null ? thru.getTime() - from.getTime() : 0;
		return d / 60/1000;
	}

	public static Date addHourToDate(Date start, int addhours) {
		Calendar cal = getCalendarInstance(TimeZone.getTimeZone("GMT"), Locale.getDefault());
	...
	public static Date getDateByHour(Date start, int hours) {
		Calendar cal = getCalendarInstance(TimeZone.getTimeZone("GMT"), Locale.getDefault());
		cal.setTimeInMillis(start.getTime());
